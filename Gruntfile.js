...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Sebastian Janzen';
    var email     = grunt.option('email')  || 'sebastian@janzen.it';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new pilight name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/pilight.png')) {
            fs.renameSync(__dirname + '/admin/pilight.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/pilight.html')) {
            fs.renameSync(__dirname + '/widgets/pilight.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/pilight/js/pilight.js')) {
            fs.renameSync(__dirname + '/widgets/pilight/js/pilight.js', __dirname + '/widgets/pilight/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/pilight')) {
            fs.renameSync(__dirname + '/widgets/pilight',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Pilight/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Pilight'
                        },
                        {
                            match: /Sebastian Janzen/g,
                            replacement: author
                        },
                        {
                            match: /sebastian@janzen.it/g,
                            replacement: email
                        }
                    ]
                },
